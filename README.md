...
- MongoDB: используется для сохранения записей QR (но история в UI хранится в `localStorage`)
...
- npm
- MongoDB (по умолчанию `mongodb://127.0.0.1:27017/qr-generator`)
- Свободные порты:
  - frontend: `3000`
  - backend: `5000`
...
> В `vite.config.js` настроен proxy, поэтому запросы вида `/api/...` проксируются на `http://localhost:5000`.
...
- UI-история реализована через `localStorage` (хранится `base64` изображение + параметры).
- Backend также пытается сохранять записи QR в MongoDB, но отдельные endpoints для получения истории не используются UI.
